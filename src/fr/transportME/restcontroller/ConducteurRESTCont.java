...
public class ConducteurRESTCont {
	...
	public ResponseEntity<Conducteur> getClient(@PathVariable("id") int idConducteur, HttpSession session) {
		...
		// return new ResponseEntity<Client>(this.clientDAO.find(((Client)
		// session.getAttribute("utilisateur")).getIdUtil()), HttpStatus.OK);
	...
	public ResponseEntity<Conducteur> modifConducteur(@PathVariable(value = "id", required = false) int idConducteur,
		...
		try {
			...
			System.out.println(
					"pbe de violation de contrainte " + e.getMessage() + " " + e.getCause() + " fin de message");
			return new ResponseEntity<Conducteur>(HttpStatus.NON_AUTHORITATIVE_INFORMATION);
		}
		if (conducteur == null) {
			System.out.println("conducteurRestController dans if conducteur null ");
			return new ResponseEntity<Conducteur>(HttpStatus.BAD_REQUEST);
		}
	...
	// getCommentsHistoryCond(@PathVariable(value="id", required=false) int
	// idConducteur) {
	// Conducteur myConducteur = this.conducteurDAO.find(idConducteur);
	// return new ResponseEntity<List<Comment>>(myConducteur.getComments(),
	// HttpStatus.OK);
	// }
	...
	@ResponseBody
	public ResponseEntity<List<Course>> getCoursesHistoryCond(
			@PathVariable(value = "id", required = false) int idConducteur) {
		Conducteur myConducteur = this.conducteurDAO.find(idConducteur);
		return new ResponseEntity<List<Course>>(myConducteur.getCourses(), HttpStatus.OK);
	}

	/**
	 * methode pour rechercher les disponibilités conducteurs autour de
	 * l'endroit où se trouve le passager
	 * 
	 * @param
	 * @return
	 */
	...
	@ResponseBody
	public ResponseEntity<List<Conducteur>> getDispoCond(@RequestParam(value = "lat", required = false) double lat,
