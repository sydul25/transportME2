...
public class HomeController {
	...
	public String inscClientPost(@Valid @ModelAttribute("client") Client client, BindingResult result,
			HttpSession session, Model model, @RequestParam String verif_password // afin
																					// de
																					// recuperer
																					// le
																					// champ
																					...
																					// classe
																					// Utilisateur
	) {// throws ErreursCoherenceException {
		...
		if (result.hasErrors()) {
			...
			// model.addAttribute("errormessage", buffer.toString()); // TODO
			// voir
			// si
			// correct
			// return "/inscription/client"; // TODO voir quel retour il faut
			// faire
		}

		// validation personnalisée
		System.out.println("appel validation du mot de passe, verif_password = " + verif_password + " password= "
				+ client.getMdpUtil());
		...
		if (result.hasErrors()) {
			...
			// System.out.println("erreurs : " + result.getFieldError());
			// System.out.println("==> retour formulaire inscription");
			// model.addAttribute("errormdp", result.getFieldError()); // TODO
			// voir
			// si
			// correct
			// return "/inscription/client"; // TODO voir quel retour il faut
		...
		if (buffer.toString().length() != 0) {
			System.out.println("erreurs suivantes: " + buffer.toString());
			return "inscription"; // "redirect:/inscription/client";
		} else

		// TODO à faire ??
		// throw new ErreursCoherenceException(buffer.toString());

		// appel service ecriture inscription que si aucune erreur de validation
		// écran
		if (!result.hasErrors()) {
			...
			System.out.println("redirection client");
			model.addAttribute("errormessage", "erreursde cohérence");
			return "inscription"; // "redirect:/inscription/client";
		}
	}

	@RequestMapping(value = "/inscription/client", method = RequestMethod.GET)
	public String inscClientGet(Model model, HttpSession session) {
		System.out.println(" inscription client");
		model.addAttribute("errormessage", "inscription client : erreurs de cohérence");
		return "inscription";
	}

	@RequestMapping(value = "/inscription/conducteur", method = RequestMethod.POST)
	public String inscConducteurPost(@Valid @ModelAttribute("conducteur") Conducteur conducteur, BindingResult result,
			HttpSession session, Model model, @RequestParam String verif_password // afin
																					// de
																					// recuperer
																					// le
		...
		if (result.hasErrors()) {
			...
			System.out.println("==> retour formulaire inscription");
			// TODO voir pour revenir sans perdre les infos saisies et mettre en
			// rouge les infos en erreur
			return "inscription";
		}

		// validation personnalisée
		System.out.println("appel validation du mot de passe, verif_password = " + verif_password + " password= "
				+ conducteur.getMdpUtil());
